...
def liga(id):
    ...
    for rows in result:
        ...
        url = "http://redoxfox1.pythonanywhere.com/resultado/" + str(idp) + "/" + str(idlig) + "/" + str(ps) + "/" + str(ps2)+ "/"
        urlrev = "http://redoxfox1.pythonanywhere.com/"
        if Jornada != nrofecha:
           Jornada = nrofecha
           cambio = 'true'
...
def form_resultados(id, liga, id_eq1, id_eq2):
    ...
    cursor = connection.cursor()
    # Read a single record
    url = "http://redoxfox1.pythonanywhere.com/actualizar/"+ str(id) + "/" + str(liga) + "/"
...
def NuevoEquipo():
    url = "http://redoxfox1.pythonanywhere.com/IngresarEquipo/"
    cursor = connection.cursor()
...
def gestion():
    url = "http://redoxfox1.pythonanywhere.com/calendario/"
    jornadas = []
    cursor = connection.cursor()
    sql = " select id from liga;"
    cursor.execute(sql)
